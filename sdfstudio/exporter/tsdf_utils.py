...
class TSDF:
    ...
    @staticmethod
    def from_aabb(aabb: torch.Tensor, volume_dims: torch.Tensor):
    ...
    def integrate_tsdf(
        ...
        color_images: Optional[torch.Tensor] = None,
...
def export_tsdf_mesh(
    ...
    c2w: torch.Tensor = cameras.camera_to_worlds.to(device)
    # make c2w homogeneous
    c2w = torch.cat([c2w, torch.zeros(c2w.shape[0], 1, 4, device=device)], dim=1)
    c2w[:, 3, 3] = 1
    K: torch.Tensor = cameras.get_intrinsics_matrices().to(device)
